document.addEventListener("DOMContentLoaded", function() {
  ...
  const handleFormSubmit = event => {
    ...
    $.ajax({
      url: 'http://121.213.241.197:81/tax',
      type: 'post',
      data: data,
      headers: {
      ...
      success: function(data) {
        ...
        console.info(data);
        window.location.href = window.location.href.replace("tax", "taxreturn/index.html?recordId=" + data.data[0]
          .recordId);
      },
      fail: function(a, b, c) {
        // console.log(a);
...
function addNumbers() {
  ...
  var sumOfAllIncome = val1 + val2 + val6 + val7 + val11 + val12 + val13 + val14 + val15 + val16 + val17 + val18 + val19 + val20 + val21 + val22 + dynamicVal;
