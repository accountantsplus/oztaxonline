...
(function () {
    ...
    ProvidersHelper.prototype.bindEventHandlers = function () {
        ...
        $('#providers').on('click', '.provider-row', function (e) {
            ...
            var html = '<div>';
            var providerIsMain = 1;
            $.each(GlobalVariables.providers, function (index, provider) {
                if(providerId != provider.id && provider.IsMain == 0 && (provider.RespondTo == providerId || provider.RespondTo == null)){
                    html +=
                        '<div class="checkbox provider_Main">' +
                ...
                if(providerId == provider.id){
                    providerIsMain = provider.IsMain;
                }
            ...
            if(providerIsMain == 1)
                $('#provider-services-linkedAccount').html(html);
            else
                $('#provider-services-linkedAccount').html("");
            
            $.each(GlobalVariables.providers, function (index, provider) {
                if(providerId == provider.id){
                    $.each(provider.relatedUsers, function(i, v){
                        $.each($(".provider_Main"), function(ind, val){
                            var inputData = $(this).find("input");
                            if(inputData.attr("data-id") == v){
                                inputData.attr("checked", "checked");
                            }
                        });
                    });
                }
            ...
            $('#provider-displayName').val(provider.screen_names);
        ...
        $('#providers').on('click', '#save-provider', function () {
            var linkedAccountData = [];
            $('.provider_Main input:checkbox').each(function () {
                if ($(this).prop('checked') == true) {
                    linkedAccountData.push($(this).attr("data-id"));
                }
            });
            ...
            var provider = {
                ...
                linkedAccount: "",
                screen_names: $('#provider-displayName').val(),
                settings: {
        ...
        $('#providers').on('click', '.display-details', function () {
            ...
            $('.working-plan-view').hide('fade', function () {
                ...
                $('.user-account-view').hide('fade');
            });
        });

        /**
         * Event: Display Provider Account Details "Click"
         */
        $('#providers').on('click', '.display-user-account', function () {
            $('#providers .switch-view .current').removeClass('current');
            $(this).addClass('current');
            $('.details-view').hide('fade', function () {
                $('.user-account-view').show('fade');
                $('.working-plan-view').hide('fade');
            });
        });
