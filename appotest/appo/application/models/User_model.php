...
class User_Model extends CI_Model {
    ...
    public function check_logins($username, $password)
    {
        $this->load->helper('general');
        $usern = hash('sha256',($username));
        $passp = hash('sha256',($password));
        
        $user_data = $this->db
            ->select('ea_users.id AS user_id, ea_users.email AS user_email, ea_users.first_name AS firstname, ea_users.last_name AS lastname, ea_users.phone_number AS mobile,'
                . 'ea_roles.slug AS role_slug')
            ->from('ea_users')
            ->join('ea_roles', 'ea_roles.id = ea_users.id_roles', 'inner')
            ->join('ea_credentials', 'ea_credentials.id = ea_users.id')
            ->where('ea_credentials.username', $usern)
            ->where('ea_credentials.password', $passp)
            ->get()->row_array();
