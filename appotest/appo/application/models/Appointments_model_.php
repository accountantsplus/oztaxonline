...
class Appointments_Model extends CI_Model {
    ...
    public function payment_update($payment_info)
    {
        $this->db->set($payment_info);
        $this->db->where('id', $payment_info['id']);
        if ( ! $this->db->update('ea_appointments', $payment_info))
        {
            throw new Exception('Could not update appointment record.');
        }
    }
    ...
    {
        $currentYear = date("Y");
        return $this->db->query('select * from ea_appointments where DATE(start_datetime) >= "'. $currentYear .'-07-01" and is_unavailable = 0')->num_rows();
    }

    public function appointment_Total_ThisWeek_count()
    {
        $currentYear = date("Y");
        $query = $this->db->query("SELECT count(*) as total FROM ea_appointments WHERE WEEK(start_datetime) = WEEK(CURDATE()) and YEAR(start_datetime) = YEAR(CURDATE()) GROUP BY CONCAT(YEAR(start_datetime), '/', WEEK(start_datetime)) and is_unavailable = 0");
        $row = $query->last_row();
        return $row->total;
    }

    public function appointment_untilToday_count()
    {
        $currentYear = date("Y");
        return $this->db->query('select * from ea_appointments where DATE(start_datetime) >= "'. $currentYear .'-07-01" and DATE(start_datetime) <= "'. date("Y-m-d") .'" and is_unavailable = 0')->num_rows();
    }

    public function appointment_untilFuture_count()
    {
        $currentYear = date("Y");
        return $this->db->query('select * from ea_appointments where DATE(start_datetime) > "'. date("Y-m-d") .'" and is_unavailable = 0')->num_rows();
    }
