...
class Appointments extends CI_Controller {
        ...
        try
        {
            ...
            foreach ($available_providers as $index => $provider)
            {
                $stripped_data = [
                    ...
                    'screen_names' => $provider['screen_names'],
                    'RespondTo' => $provider['RespondTo']
                ];
    ...
    public function book_success_update()
    {
        $post_data = $this->input->post(); 

        $this->load->model('appointments_model');

       $whatis = $this->appointments_model->payment_update($post_data);
    ...
    public function login()
    {
        ...
        $this->load->view('appointments/login');
    ...
    public function ajax_add_tempappointment()
    {
        try
        {
            ...
            $this->load->model('tempapp_model');


            $appointment['data'] = $_POST['post_data'];
            $appointment['id'] = $this->tempapp_model->add($appointment);
            $appointment['hash'] = $this->tempapp_model->get_value('hash', $appointment['id']);
    ...
    public function ajax_get_tempappointment()
    {
        try
        {
            ...
            $service = $this->tempapp_model->get_row($id);
     ...
     *   - 'dest_url'
     */
    public function ajax_simple_check_login()
    {
        try
        {
            ...
            $this->load->model('user_model');
            $user_data = $this->user_model->check_logins($this->input->post('username'), $this->input->post('password'));
