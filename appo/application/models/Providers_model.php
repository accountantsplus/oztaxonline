...
class Providers_Model extends CI_Model {
    ...
    protected function _update($provider)
    {
        ...
        for ($x = 0; $x <= count($linkedAccount); $x++) {
            $this->db->where('id', $linkedAccount[$x]);
            
            if ( ! $this->db->update('ea_users', ['RespondTo' => $provider['id']]))
            {
                throw new Exception('Could not update provider record.');
            }
        }
    ...
    public function get_row($provider_id)
    {
        ...
        // Check if selected record exists on database.
        $provider['relatedUsers'] = [];
        $relatedUsers = $this->db->get_where('ea_users', ['RespondTo' => $provider_id])->result_array();
        $provider['relatedUsers'] = [];
        foreach ($relatedUsers as $relatedUser)
        {
            $provider['relatedUsers'][] = $relatedUser['id'];
        }
    ...
    public function get_batch($where_clause = '')
    {
        ...
        foreach ($batch as &$provider)
        {
            ...
            $relatedUsers = $this->db->get_where('ea_users', ['RespondTo' => $provider['id']])->result_array();
            $provider['relatedUsers'] = [];
            foreach ($relatedUsers as $relatedUser)
            {
                $provider['relatedUsers'][] = $relatedUser['id'];
    ...
    public function get_available_providers()
    {
        ...
        $this->db
            ...
            ->join('ea_roles', 'ea_roles.id = ea_users.id_roles', 'inner')
            ->where('ea_roles.slug', DB_SLUG_PROVIDER)
            //->where('ea_users.RespondTo', null)
            ->where('ea_users.IsActive', '1');
        ...
        foreach($providers as $key => $value){
            if ($value['id'] == '4155')
            {
                $new_value = $providers[$key];
                unset($providers[$key]);
                array_unshift($providers, $new_value);
                break;
            }
        }*/
