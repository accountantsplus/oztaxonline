...
class Appointments_Model extends CI_Model {
    ...
    {
        $currentYear = date("Y");
        $query = $this->db->query("SELECT count(*) as total FROM ea_appointments WHERE WEEK(start_datetime) = WEEK(CURDATE()) GROUP BY CONCAT(YEAR(start_datetime), '/', WEEK(start_datetime)) and is_unavailable = 0");
        $row = $query->last_row();
        return $row->total;
    }
