...
class Backend_api extends CI_Controller {
    ...
    public function ajax_get_calendar_events()
    {
        try
        {
            ...
            $response = [
                ...
                'settings' => $this->settings_model->get_setting('company_working_plan')
            ];
    ...
    public function ajax_get_calendar_appointments()
    {
        try
        {
            ...
            //$response['settings'] = $this->settings_model->get_batch('id=1');,
            $response['settings'] = $this->settings_model->get_setting('company_working_plan');
    ...
    public function ajax_save_appointment()
    {
        try
        {
            ...
            try
            {
                ...
                if ((bool)$send_customer === TRUE && (bool)$appointment['SendEmail'] === TRUE)
                {
    ...
    public function ajax_filter_customers()
    {
        try
        {
            ...
            foreach ($customers as &$customer)
            {
                ...
                {
                    //echo $appointment;
                    if(is_numeric($appointment['id_services'])){
                        $appointment['service'] = $this->services_model->get_row($appointment['id_services']);   
                    }
                    
                    if(is_numeric($appointment['id_users_provider'])){
                        $appointment['provider'] = $this->providers_model->get_row($appointment['id_users_provider']);   
                    }
                }
    ...
    public function ajax_save_provider()
    {
        try
        {
            ...
            $linkedData = $provider['linkedAccount'];
