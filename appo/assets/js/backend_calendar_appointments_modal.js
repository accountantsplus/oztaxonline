...
(function(exports) {
  ...
  function _bindEventHandlers() {
    ...
    $("#manage-appointment #save-appointment").click(function() {
      ...
      var endDatetime = $dialog
        ...
        var isOverDue = false;
        if($('input[type=radio][name="overdue-Tax"]:checked').attr("id") == "overdue-Tax-yes") {
            isOverDue = true;
        }
      ...
      var appointment = {
        ...
        staff_name: $dialog.find("#staffMembers").val(),
        delivery: $dialog.find("select[name='delivery-method'] option:selected").text(),
        IsPaid: $dialog.find("#is-paid").is(":checked"),
        SendEmail: $dialog.find("#is-email-send").is(":checked"),
        is_unavailable: false,
        IsOverdueTax: isOverDue,
        TotalRentalProperty: parseInt($dialog.find("#number-years-dd").val()),
        TotalNumberofYears: parseInt($dialog.find("#rental-property-dd").val()),
        officeLocation: $dialog.find("#office-location").val()
      };
      ...
      var spouse_value, rental_value, capital_value;

      if($dialog.find('#spouse-details-yes').is(':checked'))
      {
        spouse_value = "Yes";
      }else
      {
      ...
      if($dialog.find('#rental-property-yes').is(':checked'))
      {
        rental_value = "Yes";
      }else
      {
      ...
      if($dialog.find('#capital-gains-yes').is(':checked'))
      {
        capital_value = "Yes";
      }else
      {
      ...
      var customer = {
        ...
        bsb: $dialog.find("#bsb").val(),
        bank_account: $dialog.find("#bankaccount").val(),
        tfn: $dialog.find("#tfn").val(),
        occupation_role: $dialog.find("#OccupationRole").val(),
        station_locale: $dialog.find("#station").val(),
        rank: $dialog.find("#Rank").val(),
        years_in_job: $dialog.find("#years-in-job").val(),
        spouse_firstname: $dialog.find("#spouse-firstname").val(),
        spouse_lastname: $dialog.find("#spouse-lastname").val(),
        spouse_bsb: $dialog.find("#spouse-bsb").val(),
        spouse_acc: $dialog.find("#spouse-bank").val(),
        spouse_tfn: $dialog.find("#spouse-tfn").val(),
        spouse_dob: $dialog.find("#spouse-dob").val(),
        spouse_occ: $dialog.find("#spouse-occ").val(),
        tax_year: $dialog.find("#tax-year").val(),
        spouse_yes_no: spouse_value,
        rental_property: rental_value,
        capital_gains: capital_value,
        StaffResponsible: $dialog.find("#staffMembers").val(),
      };
      ...
      function() {
        ...
        $.each(GlobalVariables.customers, function(index, c) {
          if (c.id == id) {
            ...
            $("#bsb").val(c.bsb);
            $("#bankaccount").val(c.bank_account);
            $("#tfn").val(c.tfn);
            $("#OccupationRole").val(c.occupation_role);
            $("#station").val(c.station_locale);
            $("#Rank").val(c.rank);
            $("#years-in-job").val(c.years_in_job);
            $("#spouse-firstname").val(c.spouse_firstname);
            $("#spouse-lastname").val(c.spouse_lastname);
            $("#spouse-bsb").val(c.spouse_bsb);
            $("#spouse-bank").val(c.spouse_acc);
            $("#spouse-tfn").val(c.spouse_tfn);
            $("#spouse-dob").val(c.spouse_dob);
            $("#spouse-occ").val(c.spouse_occ);
            $("#tax-year").val(c.tax_year);
            $('#OccupationalRole').val(c.occupation_role);
            $('#Rank').val(c.rank);
            $('#station').val(c.station_locale);
            $('#years-in-job').val(c.years_in_job);
            $('#tfn').val(c.tfn);
            ...
            $('#bankaccount').val(c.bank_account);
            $('#dob').val(c.dob);
            if(c.spouse_yes_no == "Yes"){
                $('#spouse-details-yes').prop('checked',true);
            }else{
                $('#spouse-details-no').prop('checked',true);
            }
            
            if(c.IsOverdueTax){
                $("#overdue-Tax-yes").prop('checked', true);
                $("#number-years-dd").val(c.TotalNumberofYears);
            } else {
                $("#overdue-Tax-no").prop('checked', true);
            }
            
            if(c.rental_property == "Yes"){
                $('#rental-property-yes').prop('checked',true);
                $("#rental-property-dd").val(c.TotalRentalProperty);
            }else{
                $('#rental-property-no').prop('checked',true);
            }
            if(c.capital_gains == "Yes"){
                $('#captial-gains-yes').prop('checked',true);
            }else{
                $('#captial-gains-no').prop('checked',true);
            }
            return false;
          }
        });
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $.each(response, function(index, c) {
            ...
            var result = $.grep(GlobalVariables.customers, function(e) {
              return e.id == c.id;
            });
    ...
    $("#rental-property-dd").keyup(function(){
        var currentVal = $(this).val();
        var prices = parseInt($("#selectedPrice").val());
        var totalRental = parseInt(currentVal) * 50;
        var totalPrice = prices * parseInt($("#number-years-dd").val());
        var grandTotalPrice = totalPrice;

        
          
        var numberYears = parseInt($("#number-years-dd").val());
        ...
        if(!isNaN(totalRental)){
            grandTotalPrice = (totalRental + prices) * numberYears;
        }
        
        var paymentUrl = "https://policetax.com.au/nab_payment.html?package=random&amount=" + parseFloat(grandTotalPrice).toFixed(2);
        $("#paymentURL").attr("href", paymentUrl);
        $("#clip-copy").html(paymentUrl);
    });
    ...
    $(document).on("change", "#number-years-dd", function(e){
        ...
        var currentVal = $(this).val();
        var prices = parseFloat($("#selectedPrice").val());
        var totalRental = parseInt($("#rental-property-dd").val()) * 50;
        var totalPrice = prices * parseInt(currentVal);
        var grandTotalPrice = totalPrice;
        
        
        if(!isNaN(totalRental)){
            grandTotalPrice = (totalRental + prices) * parseInt(currentVal);
        }
    ...
    $(document).on('change','#select-service',function(){
      ...
      $.each(GlobalVariables.availableServices, function(
        indexService,
        service
      ) {
        if (service.id == sid) {
          ...
          var prices = parseInt(service.price);
          var totalRental = parseInt($("#rental-property-dd").val()) * 50;
          var totalPrice = prices * numberYears;
          var grandTotalPrice = totalPrice;
          ...
          $("#selectedPrice").val(parseFloat(prices).toFixed(2));
      ...
      $.each(GlobalVariables.availableProviders, function(
        indexProvider,
        provider
      ) {
        $.each(provider.services, function(indexService, serviceId) {
          if (
            ...
            parseInt(provider.id) !== GlobalVariables.user.id
          ) {
      ...
      $("#manage-appointment")
        .find(
          "#customer-id, #first-name, #last-name, #email, " +
            "#phone-number, #address, #city, #zip-code, #customer-notes, #dob, #bsb, #bankaccount, #tfn, #OccupationRole, #station, #Rank, #years-in-job, #spouse-firstname, #spouse-lastname, #spouse-bsb, #spouse-bsb, #spouse-bank, #spouse-tfn, #spouse-dob, #spouse-occ, #tax-year"
        )
        .val("");
    });
  }
  ...
  exports.resetAppointmentDialog = function() {
    ...
    var startDatetime = new Date().addMinutes(
      GlobalVariables.bookAdvanceTimeout
    );
    var endDatetime = new Date()
      .addMinutes(GlobalVariables.bookAdvanceTimeout)
