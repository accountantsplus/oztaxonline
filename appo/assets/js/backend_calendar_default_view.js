...
(function (exports) {
    ...
    function _bindEventHandlers() {
        ...
        $calendarPage.on('click', '.edit-popover', function () {
            ...
            if (lastFocusedEventData.data.is_unavailable == false) {
                ...
                $dialog.find('#staffMembers').val(appointment.staff_name);

                if(appointment.IsPaid == "1"){
                    $dialog.find('#is-paid').attr("checked", true);
                    $dialog.find("#pay-appointment").hide();
                }else{
                    $dialog.find('#is-paid').attr("checked", false);
                    $dialog.find("#pay-appointment").show();
                }

                if(appointment.SendEmail == "1"){
                    $dialog.find('#is-email-send').attr("checked", true);
                }else{
                    $dialog.find('#is-email-send').attr("checked", false);
                }
                ...
                $dialog.find('#city').val(customer.suburb);
                $dialog.find('#zip-code').val(customer.zip_code);
                $dialog.find('#appointment-notes').val(appointment.notes);
                $dialog.find('#office-location').val(appointment.officeLocation);
                $dialog.find('#customer-notes').val(customer.notes);
                $dialog.find('#spouse-firstname').val(customer.spouse_firstname);
                $dialog.find('#spouse-lastname').val(customer.spouse_lastname);
                $dialog.find('#spouse-dob').val(customer.spouse_dob);
                $dialog.find('#spouse-tfn').val(customer.spouse_tfn);
                $dialog.find('#spouse-occ').val(customer.spouse_occ);
                $dialog.find('#tax-year').val(customer.tax_year);
                $dialog.find('#OccupationalRole').val(customer.occupation_role);
                $dialog.find('#Rank').val(customer.rank);
                $dialog.find('#station').val(customer.station_locale);
                $dialog.find('#years-in-job').val(customer.years_in_job);
                $dialog.find('#tfn').val(customer.tfn);
                $dialog.find('#bsb').val(customer.bsb);
                $dialog.find('#bankaccount').val(customer.bank_account);
                $dialog.find('#dob').val(customer.dob);
                if(customer.spouse_yes_no == "Yes"){
                    $dialog.find('#spouse-details-yes').prop('checked',true);
                }else{
                    $dialog.find('#spouse-details-no').prop('checked',true);
                }
                
                if(customer.capital_gains == "Yes"){
                    $dialog.find('#captial-gains-yes').prop('checked',true);
                }else{
                    $dialog.find('#captial-gains-no').prop('checked',true);
                }
                
                
                if(appointment.IsOverdueTax == 1){
                    $dialog.find("#overdue-Tax-yes").prop('checked', true);
                    $dialog.find("#number-years-dd").removeAttr("disabled");
                    $dialog.find("#number-years-dd").val(appointment.TotalNumberofYears);
                } else {
                    $dialog.find("#overdue-Tax-yes").prop('checked', false);
                    $dialog.find("#number-years-dd").attr("disabled", "disabled");
                    $dialog.find("#overdue-Tax-no").prop('checked', true);
                }
                
                if(customer.rental_property == "Yes"){
                    $dialog.find(".number-of-rental-property").show();
                    $dialog.find('#rental-property-yes').prop('checked',true);
                    $dialog.find("#rental-property-dd").val(appointment.TotalRentalProperty);
                }else{
                    $dialog.find('#rental-property-yes').prop('checked',false);
                    $dialog.find(".number-of-rental-property").hide();
                    $dialog.find('#rental-property-no').prop('checked', true);
                }
            ...
            $dialog.find('#tax-year').val($dialog.find("#tax-year option:first").val());
            $('#step-1-checked').show();
            $('#step-1-warning').hide();
    ...
    function _calendarEventClick(event, jsEvent, view) {
        ...
        if ($(this).hasClass('fc-unavailable') || $parent.hasClass('fc-unavailable') || $altParent.hasClass('fc-unavailable')) {
            ...
            displayDelete = (GlobalVariables.user.privileges.appointments.delete == true)
                ...
                var SanitiseDate = event.data.customer.dob;
                SanitiseDate = SanitiseDate.replace(/\/+/g,'-');
                var CustomerDob = new Date(SanitiseDate);
                var CurrentDate = new Date();
                var CustomerAge = Math.floor((CurrentDate-CustomerDob) / (365.25 * 24 * 60 * 60 * 1000));
                var AgeToPrint = 0;
                ...
                {
                    AgeToPrint = CustomerAge;
                }

                var isNullValueAvailable = false;
                if (event.data.customer.tfn == "")
                {
                    isNullValueAvailable = true;
                    event.data.customer.tfn = "*"
                }

                if (event.data.customer.bsb == "")
                {
                    isNullValueAvailable = true;
                    event.data.customer.bsb = "*"
                }

                if (event.data.customer.suburb == "")
                {
                    isNullValueAvailable = true;
                    event.data.customer.suburb = "*"
                }

                if (event.data.customer.station_locale == "")
                {
                    isNullValueAvailable = true;
                    event.data.customer.station_locale = "*"
                }

                if (event.data.customer.rank == "")
                {
                    isNullValueAvailable = true;
                    event.data.customer.rank = "*"
                }

                if (event.data.staff_name == null)
                {
                    isNullValueAvailable = true;
                    event.data.staff_name = "client"
                }

                var spanMandatoryMissing = "";
                if(isNullValueAvailable)
                {
                    spanMandatoryMissing = "<span style=color:red;>*</span>";
                }
                ...
                {
                    isPaid = "Yes" + ' | ' + event.data.payment_account_info + ' | ' + event.data.payment_ack_number;
                }
                if(event.data.IsSelf == "1")
                {
                    isSelf = "Yes";
                }
                
                var numberofYears = 1;
                if(parseInt(event.data.TotalNumberofYears) > 0 && !isNaN(parseInt(event.data.TotalNumberofYears))){
                    numberofYears = parseInt(event.data.TotalNumberofYears);
                }
                
                var numberofRentals = 0;
                if(parseInt(event.data.TotalRentalProperty) > 0 && !isNaN(parseInt(event.data.TotalRentalProperty))){
                    numberofRentals = parseInt(event.data.TotalRentalProperty);
                }
                
                var days = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
                var dateName = new Date(event.data.book_datetime);
                //console.log(event.data);
                var dayName = days[dateName.getDay()];
                var dateData = dateName.toString().split(' ');
            html =
                '<style type="text/css">'
                + '.popover-content strong {min-width: 80px; display:inline-block;}'
                + '.popover-content button {margin-right: 10px;}'
                + '</style>' +
                '<strong>Day Booked</strong> '
                + dayName + " " + dateData[2] + "/" + dateData[1] + "/" + dateData[3]
                + '<br>' +
                '<strong>Time Booked</strong> '
                + GeneralFunctions.formatDate(dateName, GlobalVariables.dateFormat, true)
                + '<br>' +
                '<strong>Self Book</strong> '
                + isSelf
                + '<br>' +
                '<strong>' + EALang.start + '</strong> '
                + GeneralFunctions.formatDate(event.start.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                ...
                + GeneralFunctions.formatDate(event.end.format('YYYY-MM-DD HH:mm:ss'), GlobalVariables.dateFormat, true)
                + '<br>' +
                '<strong>APT Source</strong> '
                + event.data.staff_name
                + '<br>' +
                '<strong>Office Location</strong> '
                + event.data.officeLocation
                + '<br>' +
                '<div style="width: 350px;height: 1px;border: 1px solid #666666;padding: 0px;margin: 0px;"></div> ' +
                '<strong>' + EALang.service + '</strong> '
                + event.data.service.name
                + '<br>' +
                '<strong>Method</strong> '
                + ((event.data.method !== null) ? event.data.method : '')
                + '<br>' +
                '<strong>Delivery</strong> '
                + ((event.data.delivery !== null) ? event.data.delivery : '')
                + '<br>' +
                ...
                + '<br>' +
                '<strong>' + EALang.age + '</strong> '
                + AgeToPrint
                + '<br>' +
                '<strong>Date of Birth</strong> '
                + event.data.customer.dob
                + '<br>' +
                '<strong>TFN</strong> '
                + event.data.customer.tfn
                + '<br>' +
                ...
                + '<br>' +
                '<div style="width: 350px;height: 1px;border: 1px solid #666666;padding: 0px;margin: 0px;"></div> ' +
                '<strong>Residential suburb</strong> '
                + event.data.customer.suburb
                + '<br>' +
                '<strong>Station</strong> '
                + event.data.customer.station_locale
                + '<br>' +
                '<strong>No Of Years | Rental Property</strong> '
                + numberofYears + " | " +  numberofRentals
                + '<br>' +
                '<strong>Last 3 Cc Number</strong> '
                + event.data.CreditCardNumber
                + '<br>' +
                '<strong>Receipt Number</strong> '
                + event.data.ReceiptNumber
                + '<br>' +
                '<strong>Paid</strong> '
                + isPaid 
                + '<hr>' +
    ...
    function _convertTitlesToHtml() {
        ...
        $('.fc-content').each(function () {
            var title = $.parseHTML($(this).find('.fc-title').text());
            $(this).find('.fc-title').html(title);
            var time = $(this).find('.fc-time').attr("data-full");
            $(this).find('.fc-time').html(time);
        });
    }
    ...
    function _refreshCalendarAppointments($calendar, recordId, filterType, startDate, endDate) {
        ...
        $.post(url, data, function (response) {
            ...
            var settingData = JSON.parse(response.settings);

            // Add appointments to calendar.
            var calendarEvents = [];
            var $calendar = $('#calendar');
            
            $(".currentWeekTotalAppintments").html(response.appointments.length);

            $.each(response.appointments, function (index, appointment) {
                //console.log(appointment);
                var SanitiseDate = appointment.customer.dob;
                SanitiseDate = SanitiseDate.replace(/\/+/g,'-');
                var CustomerDob = new Date(SanitiseDate);
                var CurrentDate = new Date();
                var CustomerAge = Math.floor((CurrentDate-CustomerDob) / (365.25 * 24 * 60 * 60 * 1000));
                ...
                if (appointment.customer.tfn == "")
                {
                    isMandatoryFieldMissing = true;
                    appointment.customer.tfn = "*"
                }

                if (appointment.customer.bsb == "")
                {
                    isMandatoryFieldMissing = true;
                    appointment.customer.bsb = "*"
                }

                if (appointment.customer.suburb == "")
                {
                    isMandatoryFieldMissing = true;
                    appointment.customer.suburb = "*"
                }

                if (appointment.customer.rank == "")
                {
                    isMandatoryFieldMissing = true;
                    appointment.customer.rank = "*"
                }

                if (appointment.customer.station_locale == "")
                {
                    isMandatoryFieldMissing = true;
                    appointment.customer.station_locale = "*"
                }

                var spanMandatoryMissing = "";
                if(isMandatoryFieldMissing)
                {
                    spanMandatoryMissing = "***";
                }
                
                var isPaid = '<span style="color:red;font-size:12px;">';
                if(appointment.IsPaid == "1")
                {
                    isPaid = '<span style="color:green;font-size:12px;">';
                }
                ...
                var event = {
                    id: appointment.id,
                    title: isPaid + spanMandatoryMissing + appointment.service.short_code + ' - '
                    + appointment.customer.last_name + ' '
                    + appointment.customer.first_name + ' | #' + appointment.id + ' | Age: ' + AgeToPrint + ' | '
                    + appointment.customer.rank + ' | ' + appointment.customer.station_locale + "</span>",
                    ...
                    if (provider.id == recordId) {
                        ...
                        switch (calendarView) {
                            case 'agendaDay':
                                ...
                                var currentCompanySetting = settingData[`${selectedDayName}`];
                                
                                if(currentCompanySetting != null){
                                    if(workingPlan[selectedDayName].breaks == null){
                                        
                                    } else {
                                        workingPlan[selectedDayName].breaks = currentCompanySetting.breaks;
                                    }
                                } else{
                            ...
                            case 'agendaWeek':
                                ...
                                $.each(workingPlan, function (index, workingDay) {
                                    var currentCompanySetting = settingData[`${weekDays[currentDateStart.format('d')]}`];
                                    
                                    if(currentCompanySetting != null){
                                        if(workingDay.breaks == null){
                                            
                                        } else {
                                            workingDay.breaks = currentCompanySetting.breaks;
                                        }
                                    } else{
                                        workingDay = null;
                                    }
    ...
    exports.initialize = function () {
        ...
        if (GlobalVariables.availableProviders.length > 0) {
            ...
            $.each(GlobalVariables.availableProviders, function (index, provider) {
                ...
                if(provider.RespondTo != null){
                    provider.id = provider.RespondTo;    
                }
        ...
        if (GlobalVariables.availableServices.length > 0) {
            ...
            $.each(GlobalVariables.availableServices, function (index, service) {
                if(!selectValue){
                    ...
                    optgroupHtml += '<option value="' + service.id + '" selected type="' + FILTER_TYPE_SERVICE + '">' +
                    service.name + '</option>';
                }
            });
