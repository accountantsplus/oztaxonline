...
(function () {
    ...
    CustomersHelper.prototype.bindEventHandlers = function () {
        ...
        $('#save-customer').click(function () {
            var dobData = $('#dob').val().split("/");
            var saveDOB = "";
            if(dobData.length > 1){
                saveDOB = `${dobData[2]}-${dobData[1]}-${dobData[0]}`;
            } else{
                saveDOB = dobData;
            }
            var customer = {
    ...
    CustomersHelper.prototype.display = function (customer) {
        ...
        $('#dob').val(`${dobData[2]}/${dobData[1]}/${dobData[0]}`);
        ...
        var tableVal = "No appointments available";
        var html = `<div class="appointment-row" data-id=""><table><thead style="background-color: aquamarine;"><tr><th width="20">#</th><th>Appointment No.</th><th>Has Paid?</th><th>Appointment Details</th></tr></thead><tbody>`;
        $.each(customer.appointments, function (index, appointment) {
            ...
            var isPaid = appointment.IsPaid == "1" ? "Yes" : "No";
            tableVal = "";
            html += `<tr><td style="text-align: left;vertical-align: top;"><b>${index+1})</b></td><td style="text-align: left;vertical-align: top;">${appointment.id}</td><td style="text-align: left;vertical-align: top;">${isPaid}</td><td>${start} - ${end}</td></tr>`;
            html += `<tr><td></td><td></td><td></td><td>${appointment.service.name}</td></tr>`; 
            html += `<tr><td></td><td></td><td></td><td>${appointment.provider.first_name} ${appointment.provider.last_name}</td></tr>`;
        ...
        $('#customer-appointments').html(tableVal);
