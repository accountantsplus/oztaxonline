...
class Email {
    ...
    public function sendAppointmentDetails(
        ...
        if($appointment['IsPaid'] == 0){
            //$paidVal = "but without payment";
        }
        
        $assignedUser = "Self";
        if($appointment['staff_name'] != ""){
            $assignedUser = $appointment['staff_name'];
        }
        ...
        $replaceArray = [
            ...
            '$customer_postcode' => $customer['zip_code'],
            '$booked_by' => $assignedUser,
        ...
        //$html = $html . " " . __DIR__ . '/doc/Terms_Conditions.pdf';
        $attachmentURL = __DIR__ . '/doc/Terms_Conditions.pdf';
        ...
        $mailer->Subject = $title->get() . " | " . $appointment['id'];
        $mailer->AddBCC("appoint@policetax.com.au");
        $mailer->Body = $html;
        
        
        $mailer->addStringAttachment($icsStream->get(), 'invitation.ics');
        $mailer->addAttachment($attachmentURL);
        ...
        // $paymentArray = [
         ...
         //    '$appointment_service' => $service['name'],
         //    '$appointment_price' => split("[.]",$service['price'])[0],
        //     '$appointment_paymenturl' => "https://policetax.com.au/nab_payment.html?package=random&amount=" . $service['price'],
        //     '$appointment_provider' => $provider['first_name'] . ' ' . $provider['last_name'],
          //   '$appointment_id' => $appointment['id'],
           //  '$appointment_start_date' => date($date_format . ' ' . $timeFormat, strtotime($appointment['start_datetime'])),
            // '$appointment_end_date' => date($date_format . ' ' . $timeFormat, strtotime($appointment['end_datetime'])),
      ...
      //   $payhtml = file_get_contents(__DIR__ . '/../../application/views/emails/payment_appointment.php');
       //  $payhtml = $this->_replaceTemplateVariables($paymentArray, $payhtml);
       ...
       //  $mailer = $this->_createMailer();
        ...
        // $mailer->Subject = "Payment Info | " . $appointment['id'];
        // $mailer->Body = $payhtml;
    ...
    public function sendDeleteAppointment(
        ...
        $mailer->FromName = $company['company_name'];
        $mailer->AddAddress($recipientEmail->get()); // "Name" argument crushes the phpmailer class.
        $mailer->Subject = $this->framework->lang->line('appointment_cancelled_title') . " | " . $appointment['id'];
        $mailer->Body = $html;
    ...
    public function sendPassword(NonEmptyText $password, EmailAddress $recipientEmail, array $company)
    {
        ...
        $mailer->FromName = $company['company_name'];
        $mailer->AddAddress($recipientEmail->get()); // "Name" argument crushes the phpmailer class.
        $mailer->Subject = $this->framework->lang->line('new_account_password') . " | " . $appointment['id'];
        $mailer->Body = $html;
