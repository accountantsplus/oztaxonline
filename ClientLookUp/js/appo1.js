...
function printRow() {
	...
	var hRow = $('<tr>');
	var idxes = ['Slot', 'Start Time', 'Duration', 'Client Name', 'Service', 'Rental', 'Total Rental', 'Rank', 'Station', 'Years', 'Estimate', 'Number', 'Done' ];
	...
	$.each( clients, function( index, value ){
		...
		var totalRentalCell = $('<td>').html('<span>' + value.TotalRentalProperty + '</span>');
		tRow.append(totalRentalCell);
		...
		tRow.append(attCell);
		var checked = (value.is_done == '1') ? 'checked' : '';
		var doneCell = $('<td>').html('<div class="checkbox"><a class="js-sendmail-modal-trigger" data-toggle="modal" href="#" data-index="'+index+'"><label class="checkbox-inline checkbox-bootstrap checkbox-lg"><input type="checkbox" ' + checked + '><span class="checkbox-placeholder" data-iduser="'+value.id_users_customer+'" data-field="is_done" id=' + value.id + '></span>Email</label></a></div>');
	...
	$(".checkbox-placeholder").click(function() {
		...
		$.ajax({
			...
			data: { 'field': field, 'field_value': field_value, 'id': id, 'iduser': iduser, 'rank': rank, 'station': station, 'years': years, 'estimate' : estimate },
			type: 'post',
...
function printRowNewAppo() {
	...
	$.each( clients, function( index, value ){
		...
		var promoCell = $('<td>').html('<div class="checkbox"><label class="checkbox-inline checkbox-bootstrap checkbox-lg"><input type="checkbox" ' + promo + '><span class="checkbox-placeholder" data-field="promo" id=' + value.id + '></span>Promo</label></div>');
		tRow.append(promoCell);
		var wre = (value.wre == '1') ? 'checked' : '';
		var wreCell = $('<td>').html('<div class="checkbox"><label class="checkbox-inline checkbox-bootstrap checkbox-lg"><input type="checkbox" ' + wre + '><span class="checkbox-placeholder" data-field="wre" id=' + value.id + '></span>WRE</label></div>');
		tRow.append(wreCell);
		var pf = (value.pf == '1') ? 'checked' : '';
		var pfCell = $('<td>').html('<div class="checkbox"><label class="checkbox-inline checkbox-bootstrap checkbox-lg"><input type="checkbox" ' + pf + '><span class="checkbox-placeholder"  data-field="pf" id=' + value.id + '></span>PF</label></div>');		
		tRow.append(pfCell);
		var bsb = (value.bsb == '1') ? 'checked' : '';
		var bsbCell = $('<td>').html('<div class="checkbox"><label class="checkbox-inline checkbox-bootstrap checkbox-lg"><input type="checkbox" ' + bsb + '><span class="checkbox-placeholder"  data-field="bsb" id=' + value.id + '></span>BSB</label></div>');
		tRow.append(bsbCell);
